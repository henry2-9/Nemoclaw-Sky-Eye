...
**Architecture:** R2 級聯 —— hermes cron 每週期跑便宜的 Falcon 感知 sweep 掃 16 路模擬攝影機;有候選才喚起 Nemotron-Omni 調查+分級 agent(用既有 5 個 `sentinel-*` 工具當手腳);所有對外行為一律經 `nemoclaw-act` 政策閘(信心/去重/分級路由/allowlist、PII 馬賽克、接地/防注入、限流),並留稽核軌跡。複用既有 Security-AI-Agent / Sentinel 約 80% 資產。

**Tech Stack:** Python 3 (stdlib + requests + opencv-python + pytest)、ffmpeg/ffprobe、hermes-agent(cron + script injection)、vLLM(Nemotron-Omni @ `:31010`)、Falcon Perception(HTTP `/infer`)、MongoDB(`NemoClaw_test_db`)、Telegram Bot API。
...
```bash
python3 -m venv --system-site-packages ~/.venvs/nemoclaw
NPY=~/.venvs/nemoclaw/bin/python
...
```
Expected: 印出 `deps ok 4.x`。所有測試/工具用此 venv python(`~/.venvs/nemoclaw/bin/python`)。
備註(Task 1 實測發現):
- host 上直接跑 `sentinel-*` 工具會 `from database import ...`,需 `pymongo`,且 **MongoDB 必須在跑** channel 解析才會成功(docker 容器內本來就有,host 執行才需補裝)。
- `nemoclaw.env` 已把 `~/.venvs/nemoclaw/bin` 置於 `PATH` 最前,使 `sentinel-*` 的 `#!/usr/bin/env python3` shebang 解析到此 venv。
- 用 `opencv-python-headless`(server 端免 libGL)。

- [ ] **P2: 確認分支**

Run: `git -C ~/Security-AI-Agent rev-parse --abbrev-ref HEAD`
...
Run:
```bash
bash ~/vllm-nemotron-omni-nvfp4.sh 2>/dev/null || true
...
set -euo pipefail
VID="~/sentinel-workspace/video/火煙偵測1.mp4"
...
- [ ] **Step 4: 確認 `sentinel-analyze-video` 可走 Nemotron**(env 切換驗證)

Run:
```bash
cd ~/Security-AI-Agent/tools
VLM_API_URL=http://127.0.0.1:31010/v1/chat/completions VLM_MODEL=nemotron_3_nano_omni \
SENTINEL_WORKSPACE=~/sentinel-workspace \
python3 sentinel-analyze-video.py --channel 1 --question "畫面中發生什麼事?" 2>&1 | head -20 || \
...
video_dir: ~/sentinel-workspace/video
channels:
...
export SENTINEL_WORKSPACE=~/sentinel-workspace
...
export NEMOCLAW_DB=NemoClaw_test_db
export NEMOCLAW_AUDIT_PATH=~/Security-AI-Agent/nemoclaw/audit.jsonl
export NEMOCLAW_DIR=~/Security-AI-Agent/nemoclaw
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_register.py -v`
...
def main():
    sys.path.insert(0, os.environ.get("SENTINEL_WORKSPACE", "~/sentinel-workspace"))
...
cd tools && python3 sentinel-analyze-video.py --channel 5 --extract-frame 1
```
...
git commit -m "feat(nemoclaw): register 16 Sentinel videos as simulated channels"
```
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_feed.py -v`
...
Run:
```bash
cd ~/Security-AI-Agent && python3 -c "
import nemoclaw.feed as f
p=f.grab_frame('~/sentinel-workspace/video/火煙偵測1.mp4','/tmp/feed_test.jpg', second=2)
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_falcon_client.py -v`
Expected: FAIL。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_sweep.py -v`
Expected: FAIL。
...
Run:
```bash
cd ~/Security-AI-Agent && source nemoclaw/nemoclaw.env && ./nemoclaw/nemoclaw-sweep
```
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_policy.py -v`
Expected: FAIL。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_audit.py -v`
Expected: FAIL。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_redact.py -v`
Expected: FAIL。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_notify.py -v`
Expected: FAIL。
...
Run:
```bash
cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_notify.py -v
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_act.py -v`
Expected: FAIL。
...
./nemoclaw/nemoclaw-act --incident '{"channel":"7","event_type":"fire_smoke","confidence":0.9,"severity":"high","summary":"測試濃煙","evidence_citations":[{"tool":"sentinel-analyze-video","finding":"濃煙"}]}'
...
1. `sentinel-analyze-video --channel <id> --question "畫面是否有<event_type 對應危害>?描述人員/環境並說明依據。"` — Nemotron 多模態確認與描述。
2. `sentinel-perception --channel <id> --query "<相關物件>" --task segmentation` — 取物件/分割細節證據。
3. `sentinel-event-query --camera <id> --latest` — 查近期同類事件(跨時間/跨機關聯與去重參考)。
...
```
Expected: agent 對候選依序呼叫 `sentinel-analyze-video`/`sentinel-perception`/`sentinel-event-query`,最後呼叫 `nemoclaw-act`;無候選時只回 `[SILENT]`。
備註:`hermes run` 實際旗標以 `hermes run --help` 為準;此步重點在確認 prompt 能驅動多步工具編排並收斂到 `nemoclaw-act`。需確保 hermes 的 exec allowlist 已含 `sentinel-*` 與 `nemoclaw-act`、且**移除 agent 直接 telegram/通知能力**(改於 Task 11 設定)。
...
`sentinel-analyze-video, sentinel-perception, sentinel-event-query, sentinel-violation-report, nemoclaw-act`
並**移除** `sentinel-video-ingest` 的 `--notify` 路徑與任何直接 telegram/line 工具(對外只能經 `nemoclaw-act`)。把 `nemoclaw/` 加入 `pathPrepend`。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/test_eval.py -v`
Expected: FAIL。
...
set -euo pipefail
SRC="~/sentinel-workspace/video/火煙偵測1.mp4"
OUT="~/sentinel-workspace/video/火煙偵測_注入測試.mp4"
ffmpeg -y -i "$SRC" -vf \
...
cd ~/Security-AI-Agent && source nemoclaw/nemoclaw.env
python3 -c "
import sys; sys.path.insert(0,'$SENTINEL_WORKSPACE')
from database import StreamSourceDatabase
StreamSourceDatabase().add_file_channel('Cam17-注入測試','~/sentinel-workspace/video/火煙偵測_注入測試.mp4',17,'demo')"
...
services:
  ...
  falcon-perception:
    restart: unless-stopped
```
Run: `cd ~/Security-AI-Agent && docker compose -f docker-compose.yml -f nemoclaw/docker-compose.override.yml up -d mongodb falcon-perception`
Expected: 服務 healthy。
...
Run: `cd ~/Security-AI-Agent && python3 -m pytest nemoclaw/tests/ -v`
Expected: 全數 PASS。
