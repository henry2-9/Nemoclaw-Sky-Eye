...
想要 dashboard 資料更豐富:`nohup bash nemoclaw/nemoclaw-supervisor.sh >/dev/null 2>&1 &` 跑 1–2 分鐘。
...
| # | 時間 | 畫面 | 旁白(照唸) | 指令 / 指的重點 |
|---|---|---|---|---|
| **1** | 0:00–0:18 | 瀏覽器 dashboard 自動刷新 | 「這是 NemoClaw Sentinel,跑在**一台 GB10** 上。16 路攝影機、四類危害,7×24 自主巡檢——**沒有人在迴圈裡**。」 | 指頂部指標列:決策數 / 🛡️ NemoClaw 治理 / DEDUP / 注入阻擋,點出每 5 秒自動刷新 |
