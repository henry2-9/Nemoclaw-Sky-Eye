...
HAZARD = {
    ...
    "traffic": ("交通異常事件 —— 任一具體危害都算 confirmed=true:車禍/碰撞(任何擦撞或翻覆)、"
                "車輛拋錨擋路、逆向行駛、闖紅燈、行人或動物在車道上、嚴重塞車造成路口堵塞、"
                "施工障礙物未隔離、危險駕駛、機車/腳踏車與車輛距離過近、煙霧/淹水/路面異物。"
                "平常車流順暢 = confirmed=false。對車禍/逆向/行人在道上 = severity=high 以上"),
