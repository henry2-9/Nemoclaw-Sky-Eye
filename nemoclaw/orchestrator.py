...
HAZARD = {
    ...
    "traffic": ("交通巡檢 —— **一律 confirmed=true**,記錄當下路況(audit 留巡檢軌跡):"
                "嚴重度判斷(嚴格依下列規則):"
                "(1) 看到車禍/碰撞(任何擦撞或翻覆) → severity=critical;"
                "(2) 看到逆向行駛、行人或動物在車道上、闖紅燈 → severity=high;"
                "(3) 看到嚴重塞車/壅塞、施工障礙物未隔離、車輛拋錨 → severity=medium;"
                "(4) **其他情況(車流順暢、無明顯事故、夜景、惡劣天氣但車流正常等)= severity=low,這是預設值,務必選 low**。"
                "summary 一句描述當下路況(車流量/天氣/可見距離)。"
                "**重點**:看到正常車流時,severity 必須是 low,不要選 high 或 medium。"),
